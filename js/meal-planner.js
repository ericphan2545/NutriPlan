...
const MealPlanner = {
  ...
  userData: {
    ...
    activityLevel: null, bmi: null, tdee: null, targetCalories: null,
  },
  ...
  get foodDatabase() { return FOOD_DATABASE; },
 ...
 renderCalendar() {
  ...
  autoGenerateMeals() {
    if (!this.userData.targetCalories) {
      ...
      const confirmOpen = confirm("Bạn cần cập nhật chỉ số cơ thể trước khi tạo thực đơn tự động. Mở cài đặt ngay?");
      if(confirmOpen) {
          this.openSettingsModal();
      }
      return;
    ...
    this.days.forEach((day) => {
      this.meals.forEach((meal) => {
        ...
        const usedCategories = Object.values(currentDayMeals).filter((m) => m).map((m) => m.category);
